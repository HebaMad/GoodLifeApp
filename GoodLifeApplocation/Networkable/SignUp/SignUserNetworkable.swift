...
protocol SignUserNetworkable:Networkable  {
    ...
    func startFundRaise(mobile:String,latitude:String,longitude:String,city:String, work_type:String,amount_raise:Float,interest:[String],talent:[String],completion: @escaping (Result<BaseResponse<startFundRaise>, Error>)-> ())
...
class SignUserManager:SignUserNetworkable{
    ...
    func startFundRaise(mobile:String,latitude: String, longitude: String,city:String ,work_type: String, amount_raise: Float,interest:[String],talent:[String], completion: @escaping (Result<BaseResponse<startFundRaise>, Error>) -> ()) {
        request(target: .startFundRaise(mobile:mobile,latitude: latitude, longitude: longitude, city:city ,work_type: work_type, amount_raise: amount_raise,interest:interest,talent:talent), completion: completion)
    }
    func login(mobile: String,token:String ,completion: @escaping (Result<BaseResponse<userProfile>, Error>) -> ()) {
