...
class DashboardManager:DashboardNetworkable{
    ...
    func getCategories(opportunity_id:Int,searchTxt:String,completion: @escaping (Result<BaseResponse<DahboardCategory>, Error>) -> ()) {
        request(target: .categories(opportunity_id: opportunity_id, searchTxt: searchTxt), completion: completion)
    }
    func getResources(searchTxt: String, category_id: Int, completion: @escaping (Result<BaseResponse<DashboardResource>, Error>) -> ()) {
        request(target: .Resources(searchTxt: searchTxt, category_id: category_id), completion: completion)

    }
    
    func getMyTask(searchTxt:String,completion: @escaping (Result<BaseResponse<DashboardTask>, Error>) -> ()) {
        request(target: .getMyTask(searchTxt: searchTxt), completion: completion)
    }
    func markMyTask(taskID: Int, completion: @escaping (Result<BaseResponse<Tasks>, Error>) -> ()) {
