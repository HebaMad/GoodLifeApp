...
extension UIView: Reusable {
    ...
    private func playStopSkeleton(isOn: Bool, views: [UIView] = []) {
        let cornerRadius: Float = 8
        if views.isEmpty {
            self.isSkeletonable = isOn
            if isOn {
            ...
            views.forEach({
                $0.isSkeletonable = isOn
                if isOn {
                    $0.skeletonCornerRadius = cornerRadius
    ...
    /// Show skeleton view
    /// - Parameter views: Views for skeleton
    public func playSkeleton(for views: [UIView] = []) {
        self.playStopSkeleton(isOn: true, views: views)
    }
    
    /// Hide skeleton view
    /// - Parameter views: Views for skeleton
    public func stopSkeleton(for views: [UIView] = []) {
        self.playStopSkeleton(isOn: false, views: views)
    }
